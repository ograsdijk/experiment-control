...
def append_command_journal_entry(manager: Any, payload: Json) -> None:
    ...
    if error_value is not None:
        error_json = manager._safe_json(error_value)
...
def command_journal_status_payload(manager: Any) -> Json:
    journal = manager._command_journal
    if journal is None:
        return {
            "enabled": False,
            "path": (
                str(manager._command_journal_path)
                if manager._command_journal_path is not None
                else None
            ),
            "start_error": manager._command_journal_start_error,
        }
