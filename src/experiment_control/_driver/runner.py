...
from .stream_wrappers import build_stream_wrapper
...
def _coerce_finite_simple_value(value: Any, kind: str) -> Any:
    """Coerce an RPC scalar and reject non-finite float parameters."""
    coerced = _coerce_simple_value(value, kind)
    if kind == "float" and not math.isfinite(coerced):
...
class DeviceRunner:
    ...
    def _rpc_route_set(self, req: dict[str, Any]) -> dict[str, Any]:
        ...
        if kind is not None:
            try:
                value = _coerce_finite_simple_value(value, kind)
            except Exception:
            ...
            for param in spec.params:
                if param.name in coerced:
                    ...
                    if kind is None:
                        continue
                    try:
                        coerced[param.name] = _coerce_finite_simple_value(
