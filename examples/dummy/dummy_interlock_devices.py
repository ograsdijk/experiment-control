...
def run_manager() -> None:
    ...
    # Attach interlock process with rules
    rules_path = repo_root / "examples" / "dummy" / "interlock_temperature.yaml"
    man.add_process(
        ProcessSpec(
            ...
            argv=[
                sys.executable,
                "-m",
                "experiment_control.processes.interlock",
                "--manager-rpc",
                "tcp://127.0.0.1:6000",
                "--manager-pub",
                "tcp://127.0.0.1:6001",
...
def stop_all_managed() -> None:
    ...
    # Stop managed processes and all drivers
    rpc({"type": "process.stop", "process_id": "interlock"})
