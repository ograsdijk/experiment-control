...
def _autoload_sequence(sock: zmq.Socket, sequence_path: Path) -> None:
    if not _wait_for_sequencer_rpc_ready(sock):
        print("[dummy-frequency-trace-3ch] sequencer RPC not ready; skipping autoload")
        return
    load_resp = _rpc(
        sock,
        {
    ...
    if load_resp.get("ok", False):
        print(f"[dummy-frequency-trace-3ch] loaded sequence: {str(sequence_path)}")
        return
    err = load_resp.get("error")
    print(f"[dummy-frequency-trace-3ch] sequence autoload failed: {err!r}")
    ...
    for process_id in process_ids:
        ...
        while True:
            ...
            if resp.get("ok", False):
                print(f"[dummy-frequency-trace-3ch] started process: {process_id}")
                break
            if time.monotonic() >= deadline:
                print(
                    f"[dummy-frequency-trace-3ch] failed to start process {process_id}: {resp.get('error')!r}"
                )
                break
            time.sleep(0.2)
